...
def render_header(subtitulo=""):
    ...
    <div class="header-box">
        ...
        <h3>Primavera 2026{(" — " + subtitulo) if subtitulo else ""}</h3>
...
# ════════════════════════════════════════════════════════════════════════════
    ...
    for i, p in enumerate(preguntas):
        ...
        if p["tipo"] == "escritura":
            ...
            idx = list(opciones.keys()).index(val_actual) if val_actual in opciones else None
            sel = st.radio("Elige una opción:", etiquetas, index=idx, key=f"radio_{pid}", label_visibility="collapsed")
            if sel:
                letra = sel.split(")")[0].strip()
